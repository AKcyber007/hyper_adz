@extends('admin.layouts.app', [
    'title' => 'Create Blog Article | Hyper Adz Admin'
])
...
<div class="max-w-5xl space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-sm text-slate-555 dark:text-slate-400 mt-0.5">Publish insights and industry updates to the Hyper Adz Blog network.</p>
        </div>
    </div>

    <!-- Create Form -->
    <form method="POST" action="{{ route('admin.blogs.store') }}" id="blog-form" enctype="multipart/form-data" class="space-y-6">
        @csrf
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700/50 shadow-sm space-y-4">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title') }}" placeholder="e.g. 5 Trends in Digital Signage Marketing" required class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm text-slate-700 dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:border-[#1155CC]">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <textarea name="short_description" id="short_description" rows="3" placeholder="Brief summary of the article (visible on listings pages)..." required class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm text-slate-700 dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:border-[#1155CC]">{{ old('short_description') }}</textarea>
                ...
                <div class="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700/50 shadow-sm space-y-4">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="seo_title" id="seo_title" value="{{ old('seo_title') }}" placeholder="Default: Matches Article Title" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm text-slate-700 dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:border-[#1155CC]">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <textarea name="seo_description" id="seo_description" rows="3" placeholder="Default: Matches Short Description" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm text-slate-700 dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:border-[#1155CC]">{{ old('seo_description') }}</textarea>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700/50 shadow-sm space-y-4">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="datetime-local" name="publish_date" id="publish_date" value="{{ old('publish_date') }}" class="w-full px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:border-[#1155CC]">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="slug" id="slug" value="{{ old('slug') }}" placeholder="e.g. my-custom-article" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:border-[#1155CC]">
                        @error('slug') <span class="text-xs text-rose-600 block mt-1">{{ $message }}</span> @enderror
                    </div>

                    <!-- Featured Blog Option -->
                    <div class="flex items-center gap-2 pt-2">
                        <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured') ? 'checked' : '' }} class="w-4 h-4 text-[#1155CC] bg-slate-50 border-slate-200 rounded focus:ring-[#1155CC]">
