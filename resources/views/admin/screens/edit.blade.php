@extends('admin.layouts.app', [
    'title' => 'Edit Screen | Hyper Adz Admin'
])
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold tracking-tight text-slate-900 dark:text-white">Edit Screen: {{ $screen->screen_code }}</h1>
            <p class="text-xs text-slate-500 mt-0.5">Modify technical configurations, upload new photos, or adjust impressions metrics.</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700/50 shadow-sm overflow-hidden p-6 sm:p-8">
        <form method="POST" action="{{ route('admin.screens.update', $screen->id) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select name="location_id" required class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:border-[#1155CC] transition-all">
                            @foreach($locations as $loc)
                                <option value="{{ $loc->id }}" {{ old('location_id', $screen->location_id) == $loc->id ? 'selected' : '' }}>{{ $loc->name }} ({{ $loc->city }})</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select name="screen_type_id" required class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:border-[#1155CC] transition-all">
                            @foreach($screenTypes as $type)
                                <option value="{{ $type->id }}" {{ old('screen_type_id', $screen->screen_type_id) == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="name" value="{{ old('name', $screen->name) }}" required placeholder="e.g. Lobby Vertical Screen 01" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:border-[#1155CC] transition-all">
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="screen_identifier" value="{{ old('screen_identifier', $screen->screen_identifier) }}" placeholder="e.g. BFM-LED-01" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:border-[#1155CC] transition-all">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-5">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select name="orientation" required class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:border-[#1155CC] transition-all">
                            ...
                            <option value="Square" {{ old('orientation', $screen->orientation) === 'Square' ? 'selected' : '' }}>Square</option>
                        </select>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="resolution" value="{{ old('resolution', $screen->resolution) }}" placeholder="e.g. 1920x1080" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:border-[#1155CC] transition-all">
                    </div>

                    <!-- Width (cm) -->
                    <div class="space-y-1.5">
                        <label class="text-xs font-bold text-slate-500 dark:text-slate-400">Physical Width (cm)</label>
                        <input type="number" name="screen_width" value="{{ old('screen_width', $screen->screen_width) }}" min="0" placeholder="Width" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:border-[#1155CC] transition-all">
                    </div>

                    <!-- Height (cm) -->
                    <div class="space-y-1.5">
                        <label class="text-xs font-bold text-slate-500 dark:text-slate-400">Physical Height (cm)</label>
                        <input type="number" name="screen_height" value="{{ old('screen_height', $screen->screen_height) }}" min="0" placeholder="Height" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:border-[#1155CC] transition-all">
                    </div>
                    ...
                    <div class="space-y-1.5 md:col-span-2">
                        ...
                        <input type="text" name="supported_formats" value="{{ old('supported_formats', $screen->supported_formats) }}" required placeholder="e.g. MP4,JPG,PNG" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:border-[#1155CC] transition-all">
                    ...
                    <div class="space-y-1.5 md:col-span-2">
                        ...
                        <input type="number" name="max_video_duration" value="{{ old('max_video_duration', $screen->max_video_duration) }}" min="0" placeholder="e.g. 15" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:border-[#1155CC] transition-all">
            ...
            <div>
                <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-slate-100 dark:border-slate-750 pb-2 mb-4 font-semibold">Operations</h3>
                <div class="grid grid-cols-1 md:grid-cols-4 gap-5">
                    <!-- Daily Impressions -->
                    <div class="space-y-1.5">
                        <label class="text-xs font-bold text-slate-500 dark:text-slate-400">Daily Impressions <span class="text-red-500">*</span></label>
                        <input type="number" name="daily_impressions" value="{{ old('daily_impressions', $screen->daily_impressions) }}" required min="0" placeholder="Impressions count" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:border-[#1155CC] transition-all">
                    </div>

                    <!-- Operating Hours -->
                    <div class="space-y-1.5">
                        <label class="text-xs font-bold text-slate-500 dark:text-slate-400">Operating Hours</label>
                        <input type="text" name="operating_hours" value="{{ old('operating_hours', $screen->operating_hours) }}" placeholder="Hours" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:border-[#1155CC] transition-all">
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select name="availability_status" required class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:border-[#1155CC] transition-all">
                            <option value="available" {{ old('availability_status', $screen->availability_status) === 'available' ? 'selected' : '' }}>Available</option>
                            <option value="occupied" {{ old('availability_status', $screen->availability_status) === 'occupied' ? 'selected' : '' }}>Occupied</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-1.5">
                ...
                <textarea name="description" rows="3" placeholder="Describe the physical location details, exposure, visibility patterns..." class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:border-[#1155CC] transition-all">{{ old('description', $screen->description) }}</textarea>
            </div>

            <!-- Image Management -->
            <div>
                <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-slate-100 dark:border-slate-750 pb-2 mb-4">Manage Current Photos</h3>
                
                @if($screen->images->isNotEmpty())
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
                        @foreach($screen->images->sortBy('display_order') as $img)
                            <div class="relative rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 p-2 text-center group">
                                <img src="{{ $img->image_url }}" alt="Screen image" class="w-full aspect-video object-cover rounded-xl shadow-inner mb-2">
                                
                                <div class="flex items-center justify-between px-1 text-slate-500 dark:text-slate-400 text-xs">
                                    <!-- Primary Flag -->
                                    <label class="flex items-center gap-1.5 cursor-pointer hover:text-slate-800 dark:hover:text-slate-200">
                                    ...
                                    <label class="flex items-center gap-1.5 cursor-pointer text-red-500 hover:text-red-700">
                ...
                @else
                    <p class="text-xs text-slate-450 italic mb-4">No photos uploaded for this screen yet.</p>
                @endif

                <div class="space-y-3">
                    <label class="block text-xs font-bold text-slate-500 dark:text-slate-400">Upload Additional Photos</label>
                    <div class="flex items-center justify-center w-full">
                        <label class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-200 hover:border-[#1155CC] dark:border-slate-700 dark:hover:border-slate-650 rounded-2xl cursor-pointer bg-slate-50/50 dark:bg-slate-900/30 hover:bg-slate-50 dark:hover:bg-slate-900 transition-all">
            ...
            <div class="flex items-center justify-end gap-3 border-t border-slate-100 dark:border-slate-750 pt-6 mt-8">
                <a href="{{ route('admin.screens.index') }}" class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-755 dark:bg-slate-700 dark:hover:bg-slate-650 dark:text-slate-250 rounded-xl text-sm font-semibold transition-all">
                    Cancel
                </a>
                <button type="submit" class="px-6 py-2.5 bg-[#1155CC] hover:bg-blue-700 text-white rounded-xl text-sm font-semibold transition-all shadow-md shadow-blue-500/10 flex items-center gap-2">
                    <i class="bi bi-check-lg"></i> Update Screen
                </button>
            </div>
        </form>
    </div>
        ...
        if(files) {
            Array.from(files).forEach((file, index) => {
                ...
                reader.onload = function(e) {
                    ...
                    badge.textContent = `New Photo ${index + 1}`;
