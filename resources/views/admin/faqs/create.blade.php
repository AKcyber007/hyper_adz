@extends('admin.layouts.app', [
    'title' => 'Create FAQ | Hyper Adz Admin'
])
...
<div class="max-w-4xl space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Add New FAQ</h1>
            <p class="text-sm text-slate-550 dark:text-slate-400 mt-0.5">Publish a new question and answer resource for the public site.</p>
        </div>
    </div>

    <!-- Create Form -->
    <form method="POST" action="{{ route('admin.faqs.store') }}" id="faq-form" class="space-y-6">
        @csrf
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700/50 shadow-sm space-y-4">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="question" id="question" value="{{ old('question') }}" placeholder="e.g. How does OTP login work?" required class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm text-slate-700 dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:border-[#1155CC]">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="hidden" name="answer" id="answer-input" value="{{ old('answer') }}">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700/50 shadow-sm space-y-4">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="number" name="display_order" id="display_order" value="{{ old('display_order', 0) }}" min="0" class="w-full px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:border-[#1155CC]">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Set initial content if any old input exists
        const oldContent = document.getElementById('answer-input').value;
        if (oldContent) {
