@extends('admin.layouts.app', [
    'title' => 'Advertisers List | Hyper Adz Admin'
])
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Advertiser Profiles</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage verified advertiser accounts, company details, GST records, and account statuses.</p>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('admin.advertisers.create') }}" class="px-4 py-2 bg-[#1155CC] hover:bg-blue-700 text-white rounded-xl text-xs font-bold transition-all shadow-md shadow-blue-500/10 flex items-center gap-1.5">
                <i class="bi bi-person-plus-fill"></i> Add Advertiser Manually
            </a>
        </div>
    </div>
    ...
    <div class="flex border-b border-slate-200 dark:border-slate-800 mt-2">
        <a href="{{ route('admin.advertisers.index') }}" class="px-5 py-3 border-b-2 border-[#1155CC] text-[#1155CC] dark:text-blue-400 dark:border-blue-400 font-bold text-sm flex items-center gap-2 transition-all">
            <i class="bi bi-people-fill text-base"></i>
            <span>Verified Advertisers</span>
        </a>
        <a href="{{ route('admin.leads.index', ['lead_type' => 'advertiser']) }}" class="px-5 py-3 border-b-2 border-transparent hover:border-slate-350 dark:hover:border-slate-700 text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 font-medium text-sm flex items-center gap-2 transition-all">
            <i class="bi bi-megaphone-fill text-base"></i>
            <span>Advertiser Leads</span>
            @php
                $pendingLeadsCount = \App\Models\Lead::where('lead_type', 'advertiser')->where('status', 'pending')->count();
            @endphp
    ...
    <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700/50 shadow-sm p-5">
        <form method="GET" action="{{ route('admin.advertisers.index') }}" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-6 gap-4">
            <!-- Search Text -->
            ...
            <!-- Industry Filter -->
            <div class="space-y-1">
                <label class="text-[10px] font-bold text-slate-450 uppercase tracking-wider block">Industry</label>
                <select name="industry_id" class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl text-xs text-slate-700 dark:text-slate-200 focus:outline-none focus:border-[#1155CC] transition-all">
                    <option value="">All Industries</option>
                    @foreach($industries as $ind)
                        <option value="{{ $ind->id }}" {{ ($filters['industry_id'] ?? '') == $ind->id ? 'selected' : '' }}>{{ $ind->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="space-y-1 flex items-end gap-2">
                ...
                @if(count(array_filter($filters)) > 0)
                    <a href="{{ route('admin.advertisers.index') }}" class="p-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-650 text-slate-550 dark:text-slate-300 rounded-xl transition-all shrink-0">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700/50 shadow-sm overflow-hidden">
            ...
            <table class="w-full border-collapse text-left">
                <thead>
                    <tr class="border-b border-slate-100 dark:border-slate-750 bg-slate-50/50 dark:bg-slate-850/20">
                        ...
                        <th class="px-6 py-4 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Industry</th>
                        ...
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-850/5 transition-all text-xs text-slate-700 dark:text-slate-350">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="w-10 h-10 rounded-xl overflow-hidden border border-slate-200/60 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 flex items-center justify-center shadow-inner">
                                    @if($adv->logo_path)
                                        <img src="{{ Storage::url($adv->logo_path) }}" class="w-full h-full object-cover">
                                    @else
                                        <i class="bi bi-person text-slate-400 text-base"></i>
                                    @endif
                                </div>
                            </td>
                            ...
                            <!-- Contact Details -->
                            <td class="px-6 py-4">
                                <div class="font-medium">{{ $adv->email }}</div>
                                <div class="text-[10px] text-slate-400 font-mono mt-0.5">{{ $adv->phone }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    <a href="{{ route('admin.advertisers.show', $adv->id) }}" class="w-8 h-8 rounded-lg bg-blue-50/50 hover:bg-blue-100/50 text-[#1155CC] flex items-center justify-center transition-all border border-blue-100/20" title="View Profile details">
                                        <i class="bi bi-eye-fill"></i>
                                    </a>
                                    <a href="{{ route('admin.advertisers.edit', $adv->id) }}" class="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 hover:text-slate-700 dark:hover:text-slate-200 flex items-center justify-center transition-all border border-slate-200/50 dark:border-slate-800" title="Edit Profile Details">
                                        <i class="bi bi-pencil-square"></i>
                                    </a>
                                    <form action="{{ route('admin.advertisers.destroy', $adv->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this advertiser profile?');" class="inline">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="9" class="px-6 py-12 text-center text-slate-400">
                                <div class="max-w-xs mx-auto space-y-2 py-4">
                                    <i class="bi bi-people text-3xl text-slate-300 dark:text-slate-700"></i>
                                    <p class="font-bold text-slate-700 dark:text-slate-350">No advertisers found</p>
                                    <p class="text-[11px]">Convert approved leads or register new advertiser profiles manually.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        @if($advertisers->hasPages())
            <div class="px-6 py-4 border-t border-slate-100 dark:border-slate-750 bg-slate-50/30 dark:bg-slate-850/10">
                {{ $advertisers->appends($filters)->links() }}
            </div>
        @endif
    </div>
</div>
