...
<head>
    ...
    <title>Verify OTP | Admin | Hyper Adz</title>
    <!-- Google Fonts -->
...
<body class="h-full bg-slate-950 flex items-center justify-center p-4 relative overflow-hidden select-none">
    ...
    <div class="w-full max-w-md relative z-10 space-y-6">
        ...
        <div class="text-center space-y-2">
            ...
            <p class="text-xs text-slate-450 uppercase font-bold tracking-widest">Admin Portal Verification</p>
        </div>
        ...
        <div class="bg-slate-900/50 backdrop-blur-xl border border-slate-800/60 rounded-[32px] p-6 sm:p-8 shadow-2xl space-y-6">
            <div class="space-y-1">
                ...
                <p class="text-xs text-slate-400">A 6-digit one-time passcode was dispatched to your email <strong class="text-slate-205 font-mono">{{ $email }}</strong>.</p>
            </div>
            ...
            <form method="POST" action="{{ route('admin.login.verify.post') }}" class="space-y-4">
                @csrf
            ...
            <div class="flex items-center justify-between text-[10px] pt-2">
                <a href="{{ route('login') }}" class="font-semibold text-slate-450 hover:text-white transition-all">
