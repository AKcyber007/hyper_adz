...
@section('title', 'Edit Draft Campaign')
...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold tracking-tight text-slate-900 font-outfit">Edit Draft Campaign</h1>
            <p class="text-[11px] text-slate-500">Resume editing your campaign draft.</p>
        </div>
    </div>

    <!-- Form & Calculator layout -->
    <form action="{{ route('advertiser.my-requests.update', $campaign->id) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        @csrf
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white border border-slate-200/80 rounded-[32px] p-6 space-y-5 shadow-sm">
                ...
                <div class="grid grid-cols-1 gap-4">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="campaign_name" value="{{ $campaign->campaign_name }}" required placeholder="e.g. Summer Launch Promotion" class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-indigo-500 transition-all">
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="date" name="start_date" id="startDate" value="{{ $campaign->start_date->format('Y-m-d') }}" required min="{{ date('Y-m-d', strtotime('+1 day')) }}" class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:outline-none focus:border-indigo-500 transition-all">
                        </div>

                        <!-- End Date -->
                        <div class="space-y-1.5">
                            <label class="text-[11px] font-bold text-slate-600">End Date</label>
                            <input type="date" name="end_date" id="endDate" value="{{ $campaign->end_date->format('Y-m-d') }}" required min="{{ date('Y-m-d', strtotime('+1 day')) }}" class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:outline-none focus:border-indigo-500 transition-all">
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-2 border-t border-slate-100 pt-4">
                    ...
                    @if($campaign->creative_name)
                        <div class="mb-2 text-xs text-indigo-600 font-medium">Current: {{ $campaign->creative_name }}</div>
                    @endif
                    ...
                    <p class="text-[10px] text-slate-400">Supported formats: MP4, MOV, JPG, PNG (Max 20MB). Leave blank to keep current.</p>
                </div>
            </div>
            ...
            <div class="bg-white border border-slate-200/80 rounded-[32px] p-6 space-y-4 shadow-sm">
                ...
                <div class="space-y-2 max-h-[300px] overflow-y-auto pr-1">
                    @php
                        $selectedLocIds = $campaign->locations->pluck('id')->toArray();
                    @endphp
                    @foreach($locations as $loc)
                        @php
                            $rate = $loc->price_per_day;
                            $isChecked = in_array($loc->id, $selectedLocIds);
                        ...
                        <label class="flex items-start justify-between p-3.5 bg-slate-50 border border-slate-150 rounded-2xl {{ $isActive ? 'cursor-pointer hover:bg-slate-100/50 hover:border-slate-300' : 'opacity-60 cursor-not-allowed' }} transition-all group">
                            <div class="flex gap-3">
                                <input type="checkbox" name="locations[]" value="{{ $loc->id }}" data-rate="{{ $rate }}" {{ $isChecked ? 'checked' : '' }} {{ $isActive ? '' : 'disabled' }} class="location-checkbox mt-0.5 rounded text-indigo-600 focus:ring-indigo-500 border-slate-300 {{ $isActive ? '' : 'cursor-not-allowed' }}">
                                <div>
